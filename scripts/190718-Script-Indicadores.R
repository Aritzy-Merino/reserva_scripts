...
#Establecer direcci?n de datos
setwd("/Users/HP/Documents/Indicador_funcion_policial")

#Datos con a?os desagregados

p_load(readr)

datos_v1 <- read_csv("200210-BD-Datos_funcion_policial_2017-2019.csv")
datos_v2 <- read_csv("210112-BD-Indicador_funcion_policial_2020.csv")

#Observaciones sin NA
#datos_completo <- datos[complete.cases(datos),]
datos_v1$Vocacional_2017 <- as.numeric(as.character(datos_v1$Vocacional_2017))
datos_v1$Linea_ingreso <- as.numeric(as.character(datos_v1$Linea_ingreso))
datos_v1$Control_confianza <- as.numeric(as.character(datos_v1$Control_confianza))

datos_v1_p <- datos_v1 %>% 
  select(3:16, 18:26, 28:30)

datos_v1_sub <- datos_v1 %>% 
  select(3:7, 10:11, 14:16, 18:21, 24:26, 29:30)


datos_v2_sub <- datos_v2 %>% 
  select(1:10, 12:15, 17, 19, 21, 35:36)

datos_2020 <- rbind(datos_v1_sub, datos_v2_sub)

#Análisis exploratorio inicial de 2018
...
model1 <- lm(Percepcion_inseguridad ~ Control_confianza + Estado_fuerza + Percepcion_desempeno + 
               ...
               Linea_ingreso + Primer_respondiente + Primeros_actos + Investigacion_criminal, data = datos)
summary(model1)

model2 <- lm(Percepcion_desempeno ~ Control_confianza + Estado_fuerza + 
               Percepcion_corrupcion + Percepcion_disposicion + Promedio_horas_lab_2017 + Indepol + 
               Porcentaje_policias_armadas_2017 + Porcentaje_FASP + IMC_normado_2017 + Vocacional_2017 + 
               Linea_ingreso + Primer_respondiente + Primeros_actos + Investigacion_criminal, data = datos)
summary(model2)
...
model3 <- lm(Delitos_denunciados ~ Control_confianza + Estado_fuerza + Percepcion_desempeno + 
               Percepcion_corrupcion + Percepcion_disposicion + Promedio_horas_lab_2017 + Indepol + 
               Porcentaje_policias_armadas_2017 + Porcentaje_FASP + IMC_normado_2017 + Vocacional_2017 + 
               Linea_ingreso + Primer_respondiente + Primeros_actos + Investigacion_criminal, data = datos_v1_p)
summary(model3)


#Análisis exploratorio actual 2021#####


model4 <- lm(Delitos_denunciados ~ Estado_fuerza + Percepcion_desempeno + 
               Percepcion_corrupcion + Percepcion_disposicion + Porcentaje_FASP + 
               Linea_ingreso + Primer_respondiente + Primeros_actos + Investigacion_criminal, data = datos_2020)
summary(model4)

###
model4_v2 <- lm(Delitos_denunciados ~ Estado_fuerza + 
               Percepcion_corrupcion +  Porcentaje_FASP + 
               Linea_ingreso + Primer_respondiente + Primeros_actos + Investigacion_criminal, data = datos_2020)
summary(model4_v2)


#Análisis exploratorio con sólo los datos de 2020 de nuevo gobierno

model5 <- lm(Delitos_denunciados ~ Estado_fuerza + Percepcion_desempeno + 
               Percepcion_corrupcion + Percepcion_disposicion + Porcentaje_FASP + 
               Linea_ingreso + Primer_respondiente + Primeros_actos + Investigacion_criminal + por_prot_funcion_prevencion_reaccion + por_prot_atencion_viol_genero + por_prot_uso_legitimo_fuerza + por_prot_nac_prim_respondiente + por_prot_nac_cadena_custodia + por_prot_atencion_victimas_grupos_vulnerables , data = datos_v2)
summary(model5)
